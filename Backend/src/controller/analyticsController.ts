...
export const getDailyClicks = wrapAsyncFunction(
  async (req: Request, res: Response) => {
    logger.info("getDailyClicks endpoint hit");

    const { shortId } = req.params; 
    logger.info(`Querying for shortId: ${shortId}`);
    ...
    const clicks = await Click.aggregate([
      {
        $match: {
          shortUrl: new mongoose.Types.ObjectId(shortId),
        },
      },
      {
        $group: {
...
export const getTotalClicksForUser = wrapAsyncFunction(
  async (req: Request, res: Response) => {
...
export const getClicksBreakdown = wrapAsyncFunction(
  async (req: Request, res: Response) => {
    ...
    if (!shortUrlId) {
      return res
        .status(400)
        .json({ success: false, message: "Missing shortUrlId parameter" });
    }
